...
async fn _test_addresses_impl(_use_yield: bool) -> Result<()> {
    ...
    */
    ...
    let account = WalletAccount::from_master_xprv(_xpriv_str, false, 0).await?;
    ...
    //let account = HDWalletGen2::from_extended_public_key_str(xpublic_key).await?;
    //let xpub = account.public_key().to_string(Some(Prefix::KPUB));
    //log_trace!("\nmasterKey : {}", account.to_string().as_str());
    log_trace!("extendedPubKey : {}", account.to_string().as_str());

    //log_trace!(
    //    "\nextendedKey: {}",
    //    account.receive_wallet().to_string().as_str()
    //);
    //log_trace!("extendedPubKey: {}\n", account.receive_wallet().public_key().to_string(Some(Prefix::KPUB)));
    use workflow_core::time::Instant;
    let now = Instant::now();
    let mut receive_addresses: Vec<String> = Vec::new();
    let mut change_addresses: Vec<String> = Vec::new();
    for index in 0..5000 {
        receive_addresses.push(account.derive_receive_address(index).await?.into());
        change_addresses.push(account.derive_change_address(index).await?.into());
